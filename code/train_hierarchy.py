...
parser.add_argument('--outdir', type=str, default = "saved_models/AWA/noise_sd_0.12/", help='folder to save model and training log)')
...
def main(hierarchy):
    ...
    train_dataset = get_dataset(args.dataset, 'train', hierarchy = hierarchy)
    test_dataset = get_dataset(args.dataset, 'test', hierarchy = hierarchy)
    
    samples_weights = train_dataset.samples_weights()
    sampler = WeightedRandomSampler(weights=samples_weights, num_samples= train_dataset.pos_num() , replacement=True)
    ...
    logfilename = os.path.join(args.outdir, 'hierarchy_%d_log.txt' % hierarchy)
    ...
    for epoch in range(args.epochs):
        ...
        torch.save({
            ...
            }, os.path.join(args.outdir, 'hierarchy_%d.pth.tar'% (hierarchy)))
...
def train(loader: DataLoader, model: torch.nn.Module, criterion, optimizer: Optimizer, epoch: int, noise_sd: float):
    ...
    for i, (inputs, targets) in enumerate(loader):
        ...
        for k in range(len(top1)):
            multi_acc_log += '%s_acc/TP/TN: %.2f/%.2f/%.2f; ' % (f'hierarchy_{hierarchy}', top1[k].avg,top1[k].avg_tp,top1[k].avg_tn)
        ...
        if i % args.print_freq == 0:
            ...
            for k in range(len(top1)):
                print('%s_acc/TP/TN: %.2f (%.2f)/%.2f/%.2f; ' % (f'hierarchy_{hierarchy}', top1[k].val, top1[k].avg,top1[k].avg_tp,top1[k].avg_tn))
            print('------------------------------------------')
