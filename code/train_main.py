...
parser.add_argument('--arch', type=str, default = "res50",choices=ARCHITECTURES)
parser.add_argument('--outdir', type=str, default = "saved_models/AWA/noise_sd_0.50/", help='folder to save model and training log)')
...
#                              num_workers=args.workers, pin_memory=pin_memory)
    ...
    #### weighted sampling ####
    if args.dataset == 'AwA':
        labels = pd.value_counts(train_dataset.targets).sort_index()
    ...
    logfilename = os.path.join(args.outdir, f'main{suffix}_log.txt')
    ...
    for epoch in range(args.epochs):
        ...
        }, os.path.join(args.outdir, f'main{suffix}.pth.tar'))
